...
- Other scripts:
  ...
  - `npm run generate:og` – build the app, render the `/og/...` image templates, and export refreshed OpenGraph/Twitter share cards under `public/og/`. Commit the new images with related data/content changes. Run `npx playwright install chromium` once before the first execution to ensure the capture browser is available locally.
